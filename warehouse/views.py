...
def product_diary(request):
    items = ProduceDiary.objects.all()
    context = {}
    # 翻页信息
...
def warehouse_diary(request):
    ...
    context['warehouse_diaries'] = item
...
def apply(request,user_pk):
    ...
    if request.method == 'POST':
        ...
        if apply_form.is_valid():
            ...
            add.save()
            send_mail('Subject here', '有一个采购申请请您批准.', 'jesson3264@163.com',
...
def sanction_list(request):
    items = PurchaseList.objects.filter(sanction_staff_name=None)
...
def sanction(request, apply_pk, user_pk):
    ...
    if user.is_authenticated:
        ...
        apply_tem.save()
        send_mail('Subject here', '有一个采购申请请您处理.', 'jesson3264@163.com',
                  ['jesson3264@163.com'], fail_silently=False)
...
def buy_list(request):
    items = PurchaseList.objects.filter(~Q(sanction_staff_name=None), Q(buyer_name=None) )
    context = {}
    # 翻页信息
    paginator = Paginator(items, 10)  # 每页显示10个对象
    page_number = request.GET.get('page')
...
def buyed_list(request):
    items = PurchaseList.objects.filter(~Q(buyer_name=None))
    context = {}
    # 翻页信息
    paginator = Paginator(items, 10)  # 每页显示10个对象
    page_number = request.GET.get('page')
...
def source_diary(request):
    context = {}
    items = WarehouseSource.objects.all()
    # 翻页信息
    paginator = Paginator(items, 10)  # 每页显示10个对象
    page_number = request.GET.get('page')
    if page_number is None:
