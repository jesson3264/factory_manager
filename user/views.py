...
from .forms import LoginForm, UserAppendForm, ChangePasswordForm, StaffTypeForm, UserModifyForm, AttendenceForm, \
    UserSelectForm, AttendenceSelectForm, SalarySelectForm, UserNormalSelectForm, AttendenceNormalSelectForm, \
    SalaryNormalSelectForm
...
def user_list(request):
    ...
    context = {}
    paginator = Paginator(users, 10)  # 每页显示10个对象
    ...
    user_infos = paginator.get_page(page_number)
    page_info = {}
...
def salary_lists(request):
    ...
    # 分页信息
    paginator = Paginator(salary_lists, 10)
    page_number = request.GET.get('page')
    if page_number == None:
        page_number = 1
    page_number = int(page_number)
    ...
    context = {}
    salary_list = paginator.get_page(page_number)
    
    context['salary_lists'] = salary_list
    context['page_obj'] = page_info
...
def salary_list(request, user_pk):
    ur = get_object_or_404(User, pk=user_pk)
    i_salary_lists = Salary.objects.filter(staff_name=ur)
    context = {'salary_lists': i_salary_lists}
...
def usermodify(request, user_pk):
    ...
    if usr.is_authenticated:
        ...
        if request.method == 'POST':
            print('is post')
            usermodify_form = UserModifyForm(request.POST, request.FILES)
            if 'avatar' in request.FILES:
                print('avatar in files.')
            if usermodify_form.is_valid():
                ...
                pro.avatar = usermodify_form.cleaned_data['avatar']
                print(pro.avatar)
                user.save()
                pro.save()
                return redirect(reverse('user_list'))
            else:
                print(usermodify_form.errors)
                print('is invalid')
        usermodify_form = UserModifyForm(initial={'username': user.username, 'type': user.get_staff_type,
                                                  ...
                                                  'home': user.get_staff_home,
                                                  'nationality': user.get_staff_nationality,
                                                  'phone': user.get_staff_tel,
...
def userappend(request):
    ...
    if usr.is_authenticated:
        if request.method == 'POST':
            usermodify_form = UserAppendForm(request.POST, request.FILES)
            if usermodify_form.is_valid():
                ...
                add1 = Profile(user=add, staff_type=staff_type, staff_gender=staff_gender, staff_age=staff_age,
                               staff_home=home,
                               staff_nationality=staff_nationality, staff_tel=staff_tel, id_card=id_card,
                               salary_pre_hour=salary_pre_hour)
                add1.avatar = usermodify_form.cleaned_data['avatar']
                add1.save()
                return redirect(reverse('user_list'))
            else:
                print(usermodify_form.errors)
...
def attendence_modify(request, attendence_pk):
    ...
    if usr.is_authenticated:
        ...
        if request.method == 'POST':
            ...
            AttendenceForms = AttendenceForm(
                initial={'staff_name': Attendance_tem.staff_name, 'flag_leave': Attendance_tem.flag_leave,
...
def caculate_salary(request):
    ...
    for user in users:
        ...
        if salary_tem:
            ...
            add.save()
    # fail_silently=True，即使发送失败  也不抛异常
    send_mail('Subject', '工资已经发放，请查收.', 'jesson3264@163.com',
              ['jesson3264@163.com'], fail_silently=True,
              auth_user="jesson3264@163.com", auth_password="6496063YU6110208"
              )
